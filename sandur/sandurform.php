...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO referrals (location_source, name, email, phone, address, property, property_location, source, message)
            VALUES ('Sandur', '$name','$email','$phone','$address','$property','$property_location','$source','$message')";
...
<title>Refer a Friend - Sandur Real Estate</title>
...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; }
    ...
    input, select, textarea {
        ...
        color: var(--text-main); transition: var(--transition); outline: none;
    }
    ...
    .btn {
        ...
        color: var(--white); border: none; padding: 16px; width: 100%;
        font-size: 1.1rem; font-weight: 600; border-radius: 12px;
        cursor: pointer; transition: var(--transition);
        box-shadow: 0 10px 20px rgba(67,97,238,0.3);
    }

    .btn:hover { transform: translateY(-2px); box-shadow: 0 15px 25px rgba(67,97,238,0.4); }
    .btn:active { transform: translateY(1px); }

    .back-container { text-align: center; margin-top: 30px; width: 100%; }

    .back {
        display: inline-block; text-decoration: none;
        padding: 12px 35px; font-size: 1rem; font-weight: 600;
        color: var(--text-main); background: var(--white);
        border: 2px solid #e2e8f0; border-radius: 30px;
        transition: var(--transition); box-shadow: 0 4px 6px rgba(0,0,0,0.05);
        margin: 5px;
    }

    .back:hover {
        background: var(--bg-dark); color: var(--white);
        border-color: var(--bg-dark); transform: translateY(-3px);
    ...
    .modal-content h3 { color: var(--bg-dark); font-size: 1.5rem; margin-bottom: 15px; }
    .modal-content p { line-height: 1.6; margin-bottom: 25px; }
    ...
    .modal-btn {
        ...
        box-shadow: 0 4px 15px rgba(16,185,129,0.3); flex: 1;
    }

    .modal-btn.secondary { background: var(--primary); box-shadow: 0 4px 15px rgba(67,97,238,0.3); }
    .modal-btn.home-btn { background: linear-gradient(135deg, #f72585, #b5179e); box-shadow: 0 4px 15px rgba(247,37,133,0.35); }
    .modal-btn:hover { transform: translateY(-2px); filter: brightness(110%); }

    @media (max-width: 600px) { .container { padding: 30px 20px; } .modal-actions { flex-direction: column; } }
    @media (max-width: 768px) { body { padding: 10px; } .container { padding: 30px 20px; border-radius: 20px; } h2 { font-size: 1.75rem; } .btn { padding: 12px; } }
    @media (max-width: 480px) { .modal-actions { flex-direction: column; } .modal-btn { width: 100%; } }
</style>
</head>
<body>
...
<div class="container">
    ...
    <form id="refForm" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="property_location" value="<?php echo (!empty($prefilled_loc) ? "Sandur, " . $prefilled_loc : ''); ?>" required placeholder="Enter the full property address">
        </div>

        <div class="form-group">
            <label>How did you know about us?</label>
            <input type="text" name="source" placeholder="E.g., Social Media, Friend, Website">
        </div>

        <div class="form-group">
            <label>Message</label>
            <textarea name="message" placeholder="Add any additional details or requirements here..."></textarea>
        </div>
...
<div class="back-container">
    <a href="../sandur/sandur.php" class="back">← Back to Sandur Hub</a>
...
<div id="successPopup" class="modal-overlay">
  <div class="modal-content">
    ...
    <h3>Success!</h3>
    <p>Thank you! Your property referral has been submitted successfully to the Sandur team.</p>
...
<div id="receiptTemplate" style="display:none; padding:40px; background:white; color:black; font-family:'Outfit', sans-serif;">
    <h1 style="color:#4361ee; text-align:center; font-size:28px; margin-bottom:10px;">Sandur Real Estate</h1>
...
<script>
    ...
    function closePopup() {
        ...
        setTimeout(() => {
            ...
            window.location.href = "../sandur/sandur.php";
        }, 300);
    }
    ...
    function downloadReceipt() {
        ...
        const opt = {
          margin:       0.5,
          filename:     'Sandur_Referral_Receipt.pdf',
